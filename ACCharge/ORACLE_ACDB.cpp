...
bool ORACLE_ACDB::Open()
{
	if(!IsOpen())
	{
		...
		{
			sprintf(ErrorMsg,ERROR_MSG_ORACLE_CONNECT,m_ConnectString.c_str(),e.ErrorMessage(),e.Description());
			WriteLog(ErrorMsg,E);
			return false;
		}
	}
...
void ORACLE_ACDB::ProcessCDR(CDRINFO &cdrinfo)
{
	...
	*/
	...
	{
		//记录失败日志
		sprintf(ErrorMsg,ERROR_EXEC_ORACLE_INSERTCDR,e.ErrorMessage(),e.Description());
		WriteLog(ErrorMsg,E);
		return;
	}
}